...
function getFactorial(n) {
  ...
  return subArr.reduce((acc, curr) => acc * curr, 1);
}
...
function findFirstSingleChar(str) {
  ...
  for (let i = 0; i < str.length; i += 1) {
    ...
    filteredArray = strArray.filter((char) => currentChar === char);
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  let result = '';
  const rBreacket = isStartIncluded ? '[' : '(';
  const lBreaket = isEndIncluded ? ']' : ')';
  if (a < b) {
    result = `${rBreacket}${a}, ${b}${lBreaket}`;
  } else {
    result = `${rBreacket}${b}, ${a}${lBreaket}`;
  }
  return result;
}
...
function reverseInteger(num) {
  let result = '';
  const numToString = Array.from(num.toString());
  result = Number(numToString.reverse().join(''));
  return result;
}
...
function getDigitalRoot(num) {
  let result = 0;
  const arrOfNUmbers = Array.from(num.toString());
  const sumOfNumbers = Array.from(arrOfNUmbers.reduce((acc, el) => acc + Number(el), 0).toString());
  result = sumOfNumbers.reduce((acc, el) => acc + Number(el), 0);
  return result;
}
...
function isBracketsBalanced(str) {
  const arrOfBrackets = Array.from(str);
  const stack = [];
  const bracketsList = {
    '(': ')',
    '{': '}',
  ...
  const openingBrackets = Object.keys(bracketsList);
  let isBalanced = true;
  arrOfBrackets.forEach((char) => {
    if (openingBrackets.includes(char)) {
      stack.push(char);
    } else if (
      stack.length === 0
      || bracketsList[stack.pop()] !== char
    ) {
      isBalanced = false;
    }
  });
